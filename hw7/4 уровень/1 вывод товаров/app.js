 ...
 * @param {number} prod.id id продукта
 * @param {string} prod.name название продукта
 * @param {string} prod.price цена продукта
 * @param {string} prod.imageUrl путь до картинки товара
...
function getProductMarkup(product) {
    ...
    product.forEach(prod => {
        ...
        html += `<div>${prod.name}</div>\n`;
        html += `<img src="${prod.imageUrl}" alt="">\n`;
        html += `<div>${prod.price}</div>\n`;
        html += `<a href="https://example.com/producs/${prod.id}">Подробнее</a>\n`;
        html += `</div>\n`;
    })
